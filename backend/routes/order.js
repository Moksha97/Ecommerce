...
router.post(
  ...
  ash(async (req, res) => {
    ...
    const [address_rows] = await db.query(
      "select * from address where username = ? and aid = ?",
      [username, aid]
    );
    ...
    const [account_rows] = await db.query(
      "select * from userbank where username = ? and accountid = ?",
      [username, accountid]
    );
    ...
    for (let i = 0; i < cart_rows.products.length; i++) {
      await db.query(
        ...
        [
          cart_rows.products[i].pid,
          orderid,
          cart_rows.products[i].sid,
          cart_rows.products[i].totalprice,
          cart_rows.products[i].quantity,
        ]
      );
    }

    // delete cart
    await db.query("delete from cart where username = ?", [username]);
